		...
		if order_exist>0:
			...
			for item in order_items:
				...
				if 'COMBO' in seller_sku:
					...
					#porcentaje_descuento =combo['porcentaje_descuento_combo']  Nota: Se comenta el calculo de descuento 25/02/22
					...
					if (quantity >= stock_neto_odoo) and (etiqueta_impresion==True): #Nota: Se comenta el calculo de descuento 25/02/22: (porcentaje_descuento < 40.0) and
					...
					#porcentaje_descuento = round((diferencia * 100)/ (precio_venta_odoo*1.16),2) Nota: Se comenta el calculo de descuento 25/02/22
					print ('DIFERENCIA ENTRE PRECIO ODOO VS MELI:', diferencia, 'DESCUENTO: Ya no incluye descuento', )
					...
					if (quantity <= stock_neto_odoo) and (etiqueta_impresion==True): #Nota: Se comenta el calculo de descuento 25/02/22:  (porcentaje_descuento < 40.00) and 
						print ('SI AUTOCONFIRMAR')
						autoconfirmar = True
					else:
						print ('NO AUTOCONFIRMAR')
					...
					if product:
						...
						precio_vendido_meli_sin_iva = round((precio_vendido_meli/1.16),2)
						lineas_pedido.append( (0,0,{'product_id': product_id,'product_uom':1, 'product_uom_qty':quantity, 'price_unit': precio_vendido_meli_sin_iva, 'name': title}) ) #Nota: Se comenta el calculo de descuento 25/02/22: 'discount': porcentaje_descuento,
			...
			usuario='Moises Santiago' # Comercial
	...
	try:
		...
		for pedido in pedidos_de_carrito:
			...
			if shipping_id != None:
				...
				shipment = dict(status="null", tracking_number="0", tracking_method="null")
			...
			for item in order_items:
				...
				if 'COMBO' in seller_sku:
					...
					#-- Obteniendo el Descuento
					diferencia = round((precio_venta_odoo*1.16) - precio_vendido_meli,2)
					print('DIFERENCIA ODOO VS MELI:', diferencia)
					#porcentaje_descuento = round((diferencia * 100)/ (precio_venta_odoo*1.16),2) #Nota: Se comenta el calculo de descuento 25/02/22:
...
def make_map_meli_odoo(meli_data):
	try:
		...
		if existe_orden:
			...
			print ('ESTOS SON LOS DATOS PARA ODOO', data)
...
def procesa_pedido_normal(orden, access_token):
	try:
		...
		if orden:
			...
			print ('Resultado al intentar crear la Orden Normal: ', result, ' Order Id: ', order_id)
			if result:
