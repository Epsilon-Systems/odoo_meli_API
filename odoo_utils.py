...
def colocar_nota_a_orden_meli(so_name, order_id, access_token):
    ...
    data={"note":so_name}
    print ('Orden de venta Odoo',data)
...
def get_id_odoo_product_kit(seller_sku, title, id_order, logistic_type, unit_price_combo):
	try:
		...
		#porcentaje_descuento_combo = round((diferencia_combo * 100)/ (total_precios_productos_no_combo),2) #Nota: Se comenta el calculo de descuento 05/03/22
		...
		for sub_product in sub_product_line_ids:
			...
			lineas_pedido.append( (0,0,{'product_id': product_id,'product_uom':1, 'product_uom_qty':quantity, 'price_unit': unit_price,'name': title}) ) #Nota: Se comenta el calculo de descuento 05/03/22
		return dict(lineas_pedido=lineas_pedido, list_items=list_items, stock_neto_odoo_combo=stock_neto_odoo_combo)#Nota: Se comenta el calculo de descuento 05/03/22
